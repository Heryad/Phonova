...
namespace Phonova.Models
{
    ...
    /// Edit this file to add or update device model mappings
    /// </summary>
    public static class DeviceModelMap
    {
        /// <summary>
        /// Dictionary mapping device model identifiers to their marketing names
        /// </summary>
        ...
        /// <summary>
        /// Gets the marketing name for a device model identifier
        /// </summary>
        /// <param name="modelIdentifier">Device model identifier (e.g., "iPhone14,6")</param>
        /// <returns>Marketing name (e.g., "iPhone SE 3rd Gen") or the original identifier if not found</returns>
        public static string GetDeviceName(string modelIdentifier)
        {
            ...
            if (ModelMap.TryGetValue(modelIdentifier, out string? deviceName))
            {
                return deviceName;
            }
        ...
        /// <summary>
        /// Gets the short marketing name (without carrier/connectivity details)
        /// </summary>
        /// <param name="modelIdentifier">Device model identifier (e.g., "iPhone14,6")</param>
        /// <returns>Short marketing name (e.g., "iPhone SE 3rd Gen")</returns>
        public static string GetShortDeviceName(string modelIdentifier)
        {
            string fullName = GetDeviceName(modelIdentifier);

            // Remove carrier/connectivity details for cleaner display
            fullName = fullName.Replace(" (GSM)", "")
                               .Replace(" (CDMA)", "")
                               .Replace(" (Global)", "")
                               .Replace(" (GSM+CDMA)", "")
                               .Replace(" (WiFi)", "")
                               .Replace(" (Cellular)", "")
                               .Replace(" (WiFi+Cellular)", "")
                               .Replace(" (GSM+LTE)", "")
                               .Replace(" (CDMA+LTE)", "")
                               .Replace(" (WiFi+LTE)", "")
                               .Replace(" (China)", "")
                               .Replace(" (1TB, WiFi)", "")
                               .Replace(" (1TB, WiFi+Cellular)", "")
                               .Replace(" GSM Rev A", "");
